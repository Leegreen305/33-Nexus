...
function PhaseCard({ phase, portalPhase }: { phase: Phase | undefined; portalPhase: typeof PORTAL_PHASES[number] }) {
  ...
  return (
    <div
      style={{
        background: '#080808',
        border: `1px solid ${status === 'COMPLETE' ? 'rgba(212,212,212,0.2)' : status === 'IN_PROGRESS' ? 'rgba(0,212,255,0.2)' : '#1A1A1A'}`,
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span
          style={{
            fontFamily: 'JetBrains Mono, monospace',
            fontSize: '0.55rem',
            color: status === 'COMPLETE' ? '#D4D4D4' : '#555555',
          }}
        >
          {pct}%
        </span>
      ...
      <div
        style={{
          ...
          color: status === 'PENDING' ? '#555555' : '#FFFFFF',
        ...
        <div
          style={{
            ...
            background: status === 'IN_PROGRESS'
              ...
              : 'linear-gradient(90deg, #D4D4D4, #F0F0F0)',
...
export function PortalDashboard({
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
        <div style={{
          width: '66px', height: '66px', border: '2px solid #1A1A1A',
          borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '2rem',
        }}>
          <svg width="28" height="28" viewBox="0 0 28 28" fill="none">
            <circle cx="14" cy="14" r="12" stroke="#555555" strokeWidth="1.5" />
            <line x1="14" y1="8" x2="14" y2="16" stroke="#555555" strokeWidth="1.5" strokeLinecap="round" />
            <circle cx="14" cy="20" r="1" fill="#555555" />
          </svg>
        </div>
        <h2 style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '2.2rem', color: '#FFFFFF', marginBottom: '1rem' }}>
          No Active Project
        </h2>
        <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '1rem', color: '#555555', maxWidth: '400px' }}>
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <div style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: '0.55rem', letterSpacing: '0.2em', color: '#555555', marginBottom: '0.5rem' }}>
          WELCOME BACK
        </div>
      ...
      >
        {[
          { label: 'Current Phase', value: `${project.currentPhase}/8`, color: '#D4D4D4' },
          { label: 'Unread Messages', value: String(unreadMessages), color: unreadMessages > 0 ? '#00D4FF' : '#555555' },
          { label: 'Pending Invoices', value: String(pendingInvoices.length), color: pendingInvoices.length > 0 ? '#8B0000' : '#555555' },
          { label: 'Milestones', value: String(milestones.length), color: '#D4D4D4' },
        ].map((stat) => (
          <div
            key={stat.label}
            style={{ background: '#080808', border: '1px solid #1A1A1A', borderRadius: '0.88rem', padding: '1.25rem' }}
          >
      ...
      <motion.div
        ...
        style={{ background: '#080808', border: '1px solid #1A1A1A', borderRadius: '1.1rem', padding: '2rem' }}
      >
        <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '1.32rem', letterSpacing: '0.08em', color: '#FFFFFF', marginBottom: '1.5rem' }}>
          PROJECT PHASES
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '24px' }}>
        ...
        >
          ...
          {milestones.length === 0 ? (
            <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', color: '#555555' }}>
                ...
                <div key={m.id} className="flex gap-3">
                  <div
                    style={{
                      ...
                      borderRadius: '50%',
                      background: '#D4D4D4',
                      boxShadow: '0 0 8px rgba(212,212,212,0.5)',
                      flexShrink: 0,
                      marginTop: '6px',
                    }}
                  />
        ...
        >
          <div className="flex items-center justify-between mb-6">
            <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: '1.32rem', letterSpacing: '0.08em', color: '#FFFFFF' }}>
              MESSAGES
            </div>
            {unreadMessages > 0 && (
              <div style={{
              ...
              {messages.slice(0, 5).map((msg) => (
                ...
                >
                  <div className="flex justify-between mb-1">
                    <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.77rem', color: '#D4D4D4' }}>
                  ...
                  <p style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.77rem', color: '#555555', lineHeight: 1.5 }}>
          ...
          <div className="space-y-3">
            {invoices.map((inv) => (
              <div
                ...
                style={{ padding: '0.88rem 1rem', background: '#0C0C0C', borderRadius: '0.66rem', border: '1px solid #1A1A1A' }}
              >
                <div>
                  <div style={{ fontFamily: 'DM Sans, sans-serif', fontSize: '0.88rem', color: '#FFFFFF' }}>
                ...
                <div style={{
                  ...
                  fontSize: '0.55rem',
                  letterSpacing: '0.1em',
                  background: inv.status === 'PAID' ? 'rgba(212,212,212,0.1)' : inv.status === 'OVERDUE' ? 'rgba(139,0,0,0.2)' : 'rgba(100,100,100,0.15)',
                  color: inv.status === 'PAID' ? '#D4D4D4' : inv.status === 'OVERDUE' ? '#ff4444' : '#555555',
                  border: `1px solid ${inv.status === 'PAID' ? 'rgba(212,212,212,0.2)' : inv.status === 'OVERDUE' ? 'rgba(139,0,0,0.3)' : '#1A1A1A'}`,
                }}>
                  {inv.status}
                </div>
              </div>
